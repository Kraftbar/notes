...
// Make new peace when done falling
//   ~delete peace when done falling
//    
...
void Board::OnTimer(wxCommandEvent& event)
{
    if (pieceDoneFalling) 
    {// never goes in here
        std::cout<<"never goes in here"<<std::endl; 
        pieceDoneFalling = false;
        MakeNewPiece();
    }// never goes in here
    else
...
void Board::PieceHitBottom()
{
    ...
    if (!pieceDoneFalling){
        MakeNewPiece();
        std::cout<<"Done falling ture "<<std::endl; 
        ClearFullLines();
    }
}
